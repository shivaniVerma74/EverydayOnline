                          ...
                          children: [
                            ...
                            SingleChildScrollView(
                              child: Padding(
                                ...
                                child: Column(
                                  children: [
                                    getSingleProduct(model, PROCESSED),
                                  ],
                                ),
                              ),
                            ),
...
/*  returnable() {
  ...
  orderStatus(OrderModel model) {
    var orderItem = model.itemList![0];
...
*/
  ...
  Future<void> cancelOrder(String status, Uri api, String? id) async {
    ...
    if (_isNetworkAvail) {
      try {
        ...
        print("this is my parameters ${parameter.toString()}");
  ...
  getOrderDetails(OrderModel model) {
    print("itemmm ${model.itemList![0].listStatus!.contains(PROCESSED)}");
    print("iyemms ${model.isCancleable == "1"}");
    print("jkjaksjk ${model.isAlrCancelled == "0"}");
        ...
        child: Column(
          children: [
            ...
            model.delDate != null && model.delDate!.isNotEmpty
                ...
                : Container(),
            bankTransfer(model),
            // if (!order.listStatus!.contains(DELIVERD) &&
            //     (!order.listStatus!.contains(RETURNED)) &&
            //     order.isCancle == "1" &&
            //     order.isAlrCancelled == "0")
            if (model.itemList![0].listStatus!.contains(PROCESSED) &&
                model.isCancleable == "1" &&
                model.isAlrCancelled == "0" &&
                (!model.itemList![0].listStatus!.contains(DELIVERD)))
              Padding(
                padding: const EdgeInsets.only(left: 8.0),
                child: Align(
                    alignment: Alignment.bottomRight,
                                    ...
                                    actions: [
                                      TextButton(
                                        ...
                                        onPressed: () {
                                          ...
                                          print(
                                              "this is order id ${order.toString()} and ${updateOrderApi}");
                                          cancelOrder(CANCLED,
                                              updateOrderItemApi, order);
                                        },
                                      ),
                                      TextButton(
                                        child: Text(
